...
public class Controller {
        ...
        folderChooser.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                if(selectedDirectory == null) {
                    ...
                    try {
                        songsCounter.setText(selectedDirectory.getPath());
                        if(!(players.isEmpty())) {
                            players.clear();
    ...
    public ObservableList<Song> songsUrls(File dir)   throws Exception{
        // Create a folder for save images attached in music file
        String songImagesPath = ".\\songImages";
        File songImage = new File(songImagesPath);
        if(!songImage.exists()) {
            File songParent = songImage.getParentFile();
            songImage.mkdir();
            System.out.println("songImages folder has been created!");
        }
        if(songImage.isDirectory()) {
            if(songImage.list().length > 0) {
                String[] fileList = songImage.list();
                File fileToDelete = null;

                for(int i = 0; i < fileList.length; i++) {
                    if(songImagesPath.endsWith(File.separator)) {
                        fileToDelete = new File(songImagesPath + fileList[i]);
                    } else {
                        fileToDelete = new File(songImagesPath + File.separator + fileList[i]);
                    }

                    if(fileToDelete.isFile())
                        fileToDelete.delete();
                }
                System.out.println("songImages folder has been emptied!");
            } else {
                System.out.println("songImages folder is empty!");
            }
        } else {
            System.out.println("songImages' not a directory!");
        }
                ...
                if(name.endsWith("mp3") || name.endsWith("wav")) {
                    try {
                        ...
                        if(imageData != null) {
                            ...
                            RandomAccessFile tmpFile = new RandomAccessFile(".\\songImages\\" + song.getId() + ".jpg", "rw");
...
//        songsCounter.setText("Songs: " + players.size());
    ...
    public void takeCare() throws Exception {
        if(songTable.getSelectionModel().getSelectedItem() != null) {
            ...
            System.out.println("Checked song id " + song.getId() + " in songTable.");
            setImage("\\songImages\\" + song.getId() + ".jpg");
            playPauseSong(song);
        }
        else {
            setImage();
...
//        System.out.println("show!!");
    ...
    // When has no parameter, load default image
    ...
    // When has a parameter of the song's pic, load it
